...
foreach $line (@lines) {
  ...
  $highestValue = $registers{$registerWithHighestValue};

  if ($highestValue > $alltimeHighValue) {
    $alltimeHighValue = $highestValue;
    $alltimeHighRegister = $registerWithHighestValue;
  }
}
...
print "Register with alltime highest value: $alltimeHighRegister: $alltimeHighValue\n";
